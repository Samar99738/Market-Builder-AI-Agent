...
export function strategyToJavaScript(
  spec: StrategySpec,
  opts?: { network?: "devnet" | "mainnet"; slippageBps?: number }
) {
  ...
  for (const step of spec.steps) {
    if (step.type === "buy") {
      ...
      const tok = (step as any).token ?? (step as any).outputMint ?? "";
      ...
      lines.push(`  // Unsupported step: ${(step as any).type}`);
      lines.push(``);
    }
  }

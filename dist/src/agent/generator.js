...
const ata_1 = require("../runtime/ata");
const wsol_1 = require("../runtime/wsol");
const orca_1 = require("../runtime/orca");
const mockSwap_1 = require("../runtime/mockSwap");
    ...
    return async function run() {
        ...
        if (doExecute) {
            ...
            if (network === "devnet") {
                ...
                try {
                    await (0, solana_1.ensureDevnetFunds)(conn, signer, min);
                }
                catch (e) {
        ...
        async function ensureAtaReady(mintStr) {
            const ata = await (0, ata_1.ensureAtaForMint)(conn, signer, signer.publicKey, new web3_js_1.PublicKey(mintStr));
            ...
            if (tokenAccountInfo?.value?.data && typeof tokenAccountInfo.value.data === "object" && "parsed" in tokenAccountInfo.value.data) {
                tokenAccountData = tokenAccountInfo.value.data.parsed;
        ...
        for (const step of spec.steps) {
            if (step.type === "buy") {
                ...
                let token = undefined;
                if (step.outputMint) {
                    token = step.outputMint;
                }
                else if (step.token) {
                    token = step.token;
                }
                let amount = step.budget?.amount ?? step.amount ?? 0;
                // Fallback: if amount is missing, try amountAtomic
                if ((!amount || typeof amount !== "number" || amount <= 0) && step.amountAtomic) {
                    const parsed = parseFloat(step.amountAtomic);
                    if (!isNaN(parsed) && parsed > 0)
                        amount = parsed;
                }
                let currency = step.budget?.currency ?? "USDC";
                // Patch: If currency is TOKENS and strategy text contains 'with SOL', force to SOL
                if (currency === "TOKENS" && typeof spec === "object" && 'rawText' in spec && typeof spec.rawText === 'string' && /with\s+SOL/i.test(spec.rawText)) {
                    currency = "SOL";
                }
                ...
                if (!doExecute) {
                    try {
                        const quote = await (0, functions_1.getQuoteSummary)(currency, token, amount, 50, network);
                        results.push({
                ...
                try {
                    ...
                    const amountLamports = Math.max(1, Math.round(amount * web3_js_1.LAMPORTS_PER_SOL));
                    const feeBuffer = Math.ceil(0.02 * web3_js_1.LAMPORTS_PER_SOL);
                    const needed = currency === "SOL" ? amountLamports + feeBuffer : feeBuffer;
                    if (payerBal < needed) {
                        throw new Error(`INSUFFICIENT_SOL: wallet has ${payerBal / web3_js_1.LAMPORTS_PER_SOL} SOL, needs at least ${(needed / web3_js_1.LAMPORTS_PER_SOL).toFixed(3)} SOL to execute`);
                    }
                    ...
                    if (currency !== "SOL" && inMetaMint)
                        await ensureAtaReady(inMetaMint);
                    ...
                    let progIds = [];
                    try {
                        progIds = Array.from(new Set(tx.message.compiledInstructions?.map((ix) => tx.message.staticAccountKeys[ix.programIdIndex]?.toBase58?.() || "<unknown>") || []));
                    }
                    catch { }
                    ...
                    if (network === "devnet") {
                        ...
                        for (const id of progIds) {
                            if (knownOk.has(id))
                                continue;
                            try {
                                const info = await conn.getAccountInfo(new web3_js_1.PublicKey(id));
                                const exec = !!info?.executable;
                                if (!exec) {
                                    throw new Error(`UNSUPPORTED_PROGRAM_ID: Program ${id} is not executable on devnet. Jupiter aggregator is not deployed on devnet; cannot execute swaps via API.`);
                                }
                            }
                            catch (checkErr) {
                                throw checkErr;
                    ...
                    if (currency === "SOL") {
                        await (0, wsol_1.unwrapAllWsolIfAny)(conn, signer, signer.publicKey).catch(() => { });
                    }
                    ...
                    if (network === "devnet" && /UNSUPPORTED_PROGRAM_ID_DEVNET|UNSUPPORTED_PROGRAM_ID|Unsupported program id/i.test(String(e?.message || e))) {
                        try {
                            ...
                            if (!primaryPool)
                                throw new Error("Missing ORCA_POOL_ADDRESS for devnet Whirlpool fallback");
                            console.log(`Falling back to Whirlpool pools`);
                            // Extra: log pool account owner (program id) to help diagnose fetching issues
                            try {
                                const poolInfo = await conn.getAccountInfo(new web3_js_1.PublicKey(primaryPool));
                            ...
                            const amountAtomic = String(Math.max(1, Math.round(step.budget?.amount * 10 ** inMetaDecimals || amount * 10 ** inMetaDecimals)));
                            let sig;
                            let lastErr;
                            for (const attemptPool of [primaryPool, secondaryPool].filter(Boolean)) {
                                try {
                                    console.log(`Attempting Whirlpool swap via pool ${attemptPool}`);
                                    sig = await (0, orca_1.swapViaOrcaWhirlpool)({
                                        connection: conn,
                                        user: signer,
                                        inputMint: inMetaMint,
                                        outputMint: outMetaMint,
                            ...
                            if (!sig)
                                throw lastErr || new Error('All Whirlpool pool attempts failed');
                            results.push({
                                step: "buy", execute: true, input: currency,
                            ...
                            if (process.env.ENABLE_MOCK_SWAP === '1') {
                                try {
                                    ...
                                    const amountAtomic = String(Math.max(1, Math.round(amount * 10 ** inMetaDecimals)));
                                    const mock = await (0, mockSwap_1.performMockSwap)({
                                        connection: conn,
                                        signer: signer,
                                        inputMint: inMetaMint,
                                    ...
                                    console.log(`Mock swap success signature=${mock.signature}`);
                                    results.push({
                                        step: 'buy', execute: true, input: currency, mock: true,
                                    ...
                                    const m = `MOCK_SWAP_FAILED: ${mockErr instanceof Error ? mockErr.message : String(mockErr)}`;
                    ...
                    results.push({
                        ...
                        error_code: /INSUFFICIENT_SOL/.test(msg)
                            ? "INSUFFICIENT_SOL"
                ...
                // Handle wait step
                results.push({ step: "wait", every: step.every, unit: step.unit, simulate: !doExecute });
            }
            else {
                // Skip unsupported step types
                results.push({ step: step.type ?? "unknown", skipped: true, reason: "unsupported step type in MVP" });
            }
        }
        return results;
    };
